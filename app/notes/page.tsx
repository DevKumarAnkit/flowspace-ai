...
import { getUserCategories, getUserSettings } from "@/lib/settings-server";
...
export default async function NotesRoute({ searchParams }: { searchParams: Promise<{ note?: string }> }) {
  ...
  return (
    <AppShell title="Notes">
      <NotesPage initialNotes={notes} initialSelectedNoteId={selectedNoteId} initialCategories={categories} autoSave={settings.autoSave} aiRefine={pro && settings.aiFeatures.notesRefine} />
    </AppShell>
  );
}
