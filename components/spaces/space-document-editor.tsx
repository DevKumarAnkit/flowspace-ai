...
import { Archive, ArrowLeft, Bold, Check, ChevronRight, Copy, Download, FileText, Folder, Highlighter, Italic, Link2, List, ListChecks, ListOrdered, LoaderCircle, Mic, MoreHorizontal, MoveRight, Pencil, Redo2, RotateCcw, Save, Share2, Star, Strikethrough, Trash2, Underline as UnderlineIcon, Undo2, Unlink, X } from "lucide-react";
...
export function SpaceDocumentEditor(props: { initialPage: SpacePage; space: SpaceDetail; activeSpaces: SpaceSummary[]; autoSave: boolean }) {
  return <SpacePageRoom pageId={props.initialPage.id}><SpaceDocumentEditorInner {...props} /></SpacePageRoom>;
}

function SpaceDocumentEditorInner({ initialPage, space, activeSpaces, autoSave }: { initialPage: SpacePage; space: SpaceDetail; activeSpaces: SpaceSummary[]; autoSave: boolean }) {
  ...
  const queueSave = useCallback((nextTitle: string, nextDocument: PageDocument) => {
    ...
    setStatus("dirty");
    if (timer.current) clearTimeout(timer.current);
    if (autoSave) timer.current = setTimeout(() => void save(), 700);
  }, [autoSave, readonly, save]);
